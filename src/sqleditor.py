...
#         self.logo_label = customtkinter.CTkLabel(self.sidebar_frame, text="SQL Editor", font=customtkinter.CTkFont(size=15, weight="bold"), image=self.logo_img, compound="left")
        self.logo_label = customtkinter.CTkLabel(
            self.sidebar_frame, text="SQL Editor", font=customtkinter.CTkFont(size=15, weight="bold"))
        self.logo_label.grid(row=0, column=0, padx=20, pady=(20, 10))

        self.host = customtkinter.CTkEntry(
            self.sidebar_frame, placeholder_text="host")
        self.host.grid(row=1, column=0, columnspan=1, padx=(
            10, 10), pady=(10, 10), sticky="nsew")
        self.user = customtkinter.CTkEntry(
            self.sidebar_frame, placeholder_text="user")
        self.user.grid(row=2, column=0, columnspan=1, padx=(
            10, 10), pady=(10, 10), sticky="nsew")
        self.password = customtkinter.CTkEntry(
            self.sidebar_frame, placeholder_text="password")
        self.password.grid(row=3, column=0, columnspan=1,
                           padx=(10, 10), pady=(10, 10), sticky="nsew")
        self.database = customtkinter.CTkEntry(
            self.sidebar_frame, placeholder_text="database")
        self.database.grid(row=4, column=0, columnspan=1,
                           padx=(10, 10), pady=(10, 10), sticky="nsew")
        ...
        self.drivers = {"SQLite": "QSQLITE", "MySQL": "QMYSQL",
                        "ODBC": "QODBC", "PostgresSQL": "QPSQL", "MariaDB": "QMARIADB"}
        self.driver_optionmenu.grid(row=5, column=0, padx=10, pady=5)
        self.connect_button = customtkinter.CTkButton(
            self.sidebar_frame, text="Connect", command=self.connect_db)
        ...
        self.appearance_mode_label.grid(
            row=8, column=0, padx=20, pady=(10, 0), sticky="s")
        self.appearance_mode_optionemenu = customtkinter.CTkOptionMenu(self.sidebar_frame, values=["System", "Light", "Dark"],
                                                                       command=self.change_appearance_mode_event)
        self.appearance_mode_optionemenu.grid(
            row=9, column=0, padx=20, pady=(10, 10), sticky="s")
        self.scaling_label = customtkinter.CTkLabel(
            self.sidebar_frame, text="UI Scaling:", anchor="sw")
        self.scaling_label.grid(
            row=10, column=0, padx=20, pady=(10, 0), sticky="s")
        self.scaling_optionemenu = customtkinter.CTkOptionMenu(self.sidebar_frame, values=["80%", "90%", "100%", "110%", "120%"],
                                                               command=self.change_scaling_event)
        self.scaling_optionemenu.grid(
            row=11, column=0, padx=20, pady=(10, 20), sticky="s")

        # create main entry and button
        self.entry = customtkinter.CTkEntry(
            self, placeholder_text="Search Strings")
        ...
        self.main_button_1 = customtkinter.CTkButton(master=self, fg_color="transparent", border_width=2, text_color=(
            "gray10", "#DCE4EE"), text="Search", command=self.display_table)
        ...
        self.textbox.grid(row=0, rowspan=3, column=1, columnspan=3, padx=(
...
#         #self.main_button_1.grid(row=3, column=5, padx=(20, 20), pady=(20, 20), sticky="nsew")
    ...
    def display_table(self):
        ...
        self.column_names = [record.fieldName(
            i) for i in range(record.count())]
    ...
    def match_strings(self, s):
        ...
        query.prepare(
            f"""SELECT * FROM {self.table} WHERE ({w}) LIKE '%{s}%'""")
        query.exec()
    ...
    def connect_db(self):
        ...
        self.con = QSqlDatabase.addDatabase(
            self.drivers[self.driver_optionmenu.get()])
        ...
        if not self.con.open():
            self.Popup(
                f"Could not connect to table due to the following error:\n{self.con.lastError().databaseText()}")
        else:
            self.show_tables()
    ...
    def show_tables(self):
        ...
        self.table_buttons = [customtkinter.CTkButton(
            master=self.tables_frame, text=x, command=lambda j=x: self.get_table(j)) for x in self.con.tables()]
        for idx, btn in enumerate(self.table_buttons):
            btn.grid(row=idx // tables_per_rc, column=idx %
                     tables_per_rc, pady=(20, 10), padx=20, sticky="n")
    ...
    def show_columns(self, cols):
        if hasattr(self, 'cols_frame'):
            ...
            self.cols_frame = customtkinter.CTkFrame(self.options)
            self.cols_frame.pack(padx=10, pady=10)
        self.col_boxes = [customtkinter.CTkCheckBox(
            master=self.cols_frame, text=x) for x in cols]
        for idx, box in enumerate(self.col_boxes):
            box.select()
            box.grid(row=idx // 3, column=idx %
                     3, pady=(20, 10), padx=20, sticky="n")
